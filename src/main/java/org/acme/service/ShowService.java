...
public class ShowService implements IShowService {
    ...
    @Override
    public HandlerResponse getShowsById1(Integer showId) {
    ...
    @Override
    public HandlerResponse getShows1() {
    ...
    @Override
    public HandlerResponse getShows2() {
        ...
        try{
            ...
            ClientBuilder builder = ClientBuilder.newBuilder();
            Client client = builder.build();
            WebTarget webTarget = client.target("https://api.tvmaze.com/shows");
            Response response = webTarget.request().get();

            if(response.getStatus() == Response.Status.OK.getStatusCode()){
                String entityResponse = response.readEntity(String.class);
                System.out.println(entityResponse);
                JsonArray jsonObject = new JsonArray(entityResponse);
                ObjectMapper objectMapper = new ObjectMapper();
                objectMapper.enable(DeserializationFeature.ACCEPT_SINGLE_VALUE_AS_ARRAY);

                showList = objectMapper.readValue(jsonObject.encode(), new TypeReference<List<Show>>() {});
            }else{
                message = "Error";
                httpCode = Response.Status.NOT_FOUND.getStatusCode();
            }
